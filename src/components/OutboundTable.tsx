...
import { OutboundDeal, Destination } from "../types";
import { 
...
interface OutboundTableProps {
  ...
  onAddDeal: (deal: Omit<OutboundDeal, "id">) => void;
  onUpdateDeal: (id: string, updated: Partial<OutboundDeal>) => void;
  onDeleteDeal: (id: string) => void;
  onResetToDefaults: () => void;
}

type SortField = "maskedDestination" | "plannedMinutes" | "cost" | "plannedCost";
...
export default function OutboundTable({
  ...
  const handleQuickAdd = (e: React.FormEvent) => {
    ...
    if (isNaN(costVal) || costVal < 0) {
      setAddError("Cost/CPM rate must be a positive number.");
      return;
    }
  ...
  const sortedDeals = [...filteredDeals].sort((a, b) => {
    ...
    if (sortField === "plannedCost") {
      valA = (a.plannedMinutes || 0) * (a.cost || 0);
      valB = (b.plannedMinutes || 0) * (b.cost || 0);
    } else {
      valA = a[sortField];
      valB = b[sortField];
    }
  ...
  const totalPlannedCost = sortedDeals.reduce(
    (sum, deal) => sum + ((deal.plannedMinutes || 0) * (deal.cost || 0)),
    0
  );
  ...
  return (
    <div id="outbound-billing-card" className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full">
      {/* Header Panel */}
      <div className="p-5 border-b border-slate-50 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 bg-amber-500 rounded-full pulse-indicator"></div>
            <h2 className="text-lg font-display font-semibold text-slate-800">2. Outbound Logistics</h2>
          </div>
          <p className="text-xs text-slate-400 mt-0.5">Tracks external dispatches, outgoing minutes, and planned costs</p>
        </div>
        <div className="text-xs text-slate-400 bg-white border border-slate-200 px-3 py-1 rounded-md shadow-sm self-start font-mono">
          Cost calculations: Minutes * CPM
        </div>
      </div>

      {/* Quick Add Form Panel */}
      <div className="p-5 border-b border-slate-100 bg-white">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-3 block">Quick Add New Outbound Entry</h3>
        <form onSubmit={handleQuickAdd} className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
          ...
          <div className="space-y-1">
            ...
            <div className="flex gap-1.5">
              ...
              <div className="flex gap-1">
                <button
                  type="button"
                  onClick={() => applyDestinationMask("LHR")}
          ...
          <div className="space-y-1">
            <label className="text-[11px] font-medium text-slate-500 block">Cost (CPM / $/Min)</label>
            <input
              type="number"
              step="0.01"
              min="0"
          ...
          <div>
            <button
              type="submit"
              className="w-full bg-slate-900 hover:bg-slate-800 border-none text-white rounded-lg px-4 py-2 text-sm font-semibold flex items-center justify-center gap-1.5 shadow-sm transition-all cursor-pointer"
            >
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100 text-sm">
            {sortedDeals.length > 0 ? (
              sortedDeals.map((deal) => {
                ...
                const plannedCost = (deal.plannedMinutes || 0) * (deal.cost || 0);
                ...
                return (
                  ...
                  >
                    ...
                    {/* Planned Cost (Calculated auto-multiplier) */}
              ...
              <tr>
                <td colSpan={5} className="py-8 text-center text-slate-400 text-xs">
                  No active outbound matches found. Register high-capacity routes above.
                </td>
              </tr>
            )}
          </tbody>
          ...
          <tfoot className="border-t-2 border-slate-200 bg-slate-50/50 text-slate-800 font-semibold font-mono">
            <tr>
              ...
              <td className="py-3 px-4 text-right text-amber-700 font-extrabold text-base bg-amber-50/20">
